...
## 3. Git工作区、暂存区和版本库
  ...
  * **版本库**：工作区有一个隐藏目录.git，这个不算工作区，而是Git版本库
  ...
  ![git_flow](https://raw.githubusercontent.com/XianZhou/Jotting/master/img/jotting/git/git_flow.jpg)
...
### 4.6 git commit
  ...
  *  将工作区内容添加到版本库中，省略git add ```git commit -am "description"```，没有新添加的文件情况下可用
...
### 5.5 GitHub
  ...
  * ```git rebase``` 用于合并两个分支，可以简化分支版本的控制；功能与 ```git merge``` 类似，不同之处在于：
    ...
    * **git merge** 相当于在pull线上代码时进行了一次合并提交
      ...
      ![git_merge](https://raw.githubusercontent.com/XianZhou/Jotting/master/img/jotting/git/git_merge.jpg)

    * **git rebase** 可以让自己的分支看起来没有经过任何合并

      ![git_rebase](https://raw.githubusercontent.com/XianZhou/Jotting/master/img/jotting/git/git_rebase.jpg)
...
### 5.6 GitLab
  GitLab 是一个用于仓库管理系统的开源项目，使用Git作为代码管理工具，并在此基础上搭建起来的web服务，一些使用说明如下：
  * 首先用公司邮箱登陆GitLab，在 **设置** -> **SSH密钥** 添加该邮箱的密钥，添加   *～/.ssh/id_rsa.pub* 公钥文件的内容
  * 访问项目前，需要申请被拉入该项目的 **成员** 列表里
  * 上传代码时，一般上传到新建的个人分支，例如 *dev-zhouxian* ，然后向主分支提交合并请求，提交时需选择指派人
...
## 7. 总结
  ...
  ![git_summary](https://raw.githubusercontent.com/XianZhou/Jotting/master/img/jotting/git/git_summary.jpeg)
